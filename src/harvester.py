...
class HarvesterBL(CommonI14YAPI):
    ...
    def __init__(self, api_params):
        ...
        - identifier_pattern: re.compile object with the pattern used to identify the dataset on i14y
        """
    ...
    def fetch_datasets_from_api(self) -> List[Dict]:
        ...
        try:
            ...
            print(f"Error processing datasets:")
            traceback.print_exc()

        print(f"Successfully processed {len(datasets)} datasets")
        return datasets

    def fix_bad_resource_literals(self, g: Graph):
        for p in (DCTERMS.spatial, DCTERMS.coverage):
            for s, o in list(g.subject_objects(p)):
                if isinstance(o, URIRef) and ("://" not in str(o)):
                    g.remove((s, p, o))
    ...
    def parse_rdf_file(self, file_path):
        ...
        self.fix_bad_resource_literals(graph)
    ...
    def create_dataset_payload(self, dataset: Dict) -> Dict:
    ...
    def delete_i14y(self, dataset_id):
        ...
        try:
            ...
            self.session.delete(url_structures, headers=headers)
        except requests.HTTPError as e:
